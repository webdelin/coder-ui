...
export default defineEventHandler(async () => {
  ...
  const opts: Partial<Record<ProviderName, Record<string, string>>> = {}
  ...
  for (const s of settings) {
    ...
    if (s.baseUrl) {
      opts[s.provider as ProviderName] = { baseUrl: s.baseUrl }
    }
  }
  ...
  return fetchAllProviderModels(apiKeys, opts)
})
